...
export const CustomersModule: React.FC<CustomersModuleProps> = ({
  ...
  const handleOpenEdit = (c: Customer) => {
    ...
    setFormData({ ...c, isActive: c.isActive !== false });
    setShowModal(true);
  };
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2 bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-800 block">Estado del Cliente</span>
                    <span className="text-[11px] text-slate-500">
                      {formData.isActive !== false ? 'Cliente Activo (Habilitado para compras)' : 'Cliente Inactivo'}
                    </span>
                  </div>
